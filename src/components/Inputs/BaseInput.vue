...
const emit = defineEmits(['update:modelValue', 'cleanInput'])
</script>

<template>
  <div class="input__item">
    <div v-if="props.type === 'text'" :class="['left__icon', { disabled: disabled }]">
      <slot name="seacrh" />
    </div>
    <input
      :class="[
        'input',
        `size_${size}`,
        { searchInput: $slots.seacrh, disabled: disabled },
      ...
      v-bind="$attrs"
    />
    <div v-show="modelValue && props.type" class="input__icon" @click="emit('cleanInput')">
      <Icon type="x-circle" />
    </div>
